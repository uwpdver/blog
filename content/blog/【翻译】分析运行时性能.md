...
description: '对文章 Analyze runtime performance 的翻译'
---
...
#### JavaScript：工具  
使用 Performance 工具录制，并且发现出乎意料久的```执行脚本（Evaluate Script）``` 事件。  

如果你注意到在你的 JavaScript 执行过程中出现了相当多的卡顿（渲染的中断），你可能需要更进一步地分析并且收集一份 JavaScript CPU 概况（profiles）。CPU 概况展示了在你的页面的函数中运行时间花费在了哪里。你可以在[Speed up JavaScript runtime](https://docs.microsoft.com/en-us/microsoft-edge/devtools-guide-chromium/rendering-tools/js-runtime)中学习怎样创建 CPU 概况。  

#### JavaScript：问题  
下面这个表格描述了一些常见的 JavaScript 问题和可能的方法。  
| 问题 | 例子 | 解决方法 |
| --- | --- | --- |
| 昂贵的输入处理程序影响页面响应或者动画。 | 触摸，视差滚动 | 让浏览器处理触摸和滚动，或者尽可能晚地绑定事件监听器。 推荐阅读文章 [Expensive Input Handlers in Paul Lewis' runtime performance checklist](https://calendar.perfplanet.com/2013/the-runtime-performance-checklist/) |
| 时机不恰当的 JavaScript 影响页面响应，动画，和加载 | 用户在页面加载完成后向向右滚动，setTimeout / setInterval。 | 使用 JavaScript 运行时：用 ```requestAnimationFrame``` 将 DOM 操作分散到各帧中，使用 [Web Workers](https://developer.mozilla.org/docs/Web/API/Web_Workers_API/Using_web_workers) |
...
选择一个 ```Recalculate Style``` 事件，在 Details 面板查看更多的相关信息。如果这个样式改变花了太长时间，那这就是一个性能问题。如果这个样式计算影响了很多元素，那这就是另外一个可改进的空间。  
...
作为一般的经验法则，如果你在一帧完成之前要求 DOM 返回一个几何属性的值，你会发现你触发了 “强制同步布局”，如果经常重复对大型 DOM 树执行的话，这将是一个很大的性能瓶颈。  
